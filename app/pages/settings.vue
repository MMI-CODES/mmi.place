...
const tab = ref<"appearence" | "customization" | "widgets">(
	(route.query.tab as any) || "appearence",
);

// --- Groups for Vencat & PlanUP Smart Select ---
const baseGroups: Record<
	string,
	{ label: string; value: string; style: "neutral" }[]
> = {
	MMI1: [
...
const vencatYear = ref(
	getYearFromValue(settings.value?.widgets?.vencat?.group || null),
);
const planupYear = ref(
	getYearFromValue(settings.value?.widgets?.planup?.group || null),
);
...
watch(vencatYear, (newYear) => {
	if (!newYear) {
		...
		getYearFromValue(settings.value.widgets.vencat.group) !== newYear
	) {
		settings.value.widgets.vencat.group = baseGroups[newYear]?.[0]?.value || null;
	}
});
...
watch(planupYear, (newYear) => {
	if (!newYear) {
		...
		getYearFromValue(settings.value.widgets.planup.group) !== newYear
	) {
		settings.value.widgets.planup.group = baseGroups[newYear]?.[0]?.value || null;
	}
});
...
const increaseTextSize = () => {
	const idx = textSizes.indexOf(settings.value.appearence.text.size);
	if (idx < textSizes.length - 1)
		settings.value.appearence.text.size = textSizes[idx + 1] || "medium";
};
...
onMounted(() => {
	loadSettings();
	applySettings();
	vencatYear.value = getYearFromValue(
		settings.value.widgets.vencat.group || null,
	);
	planupYear.value = getYearFromValue(
		settings.value.widgets.planup.group || null,
	);
	fetchChannels();
});
...
<template>
	...
	<main
		v-if="tab === 'appearence'"
		class="container flex flex-col gap-10 max-w-2xl mx-auto"
	>
		<section class="flex flex-col gap-4">
			<h2 class="text-2xl font-bold border-b border-surface-border pb-2">
				Apparence Globale
			</h2>
			<div
			...
			>
				<div class="flex items-center gap-3 font-medium">
					<MoonIcon
						class="w-6 h-6 text-primary/70"
						v-if="settings.appearence.theme === 'dark'"
					/>
				...
				<Switch
					...
					@update:modelValue="
						(val) =>
							(settings.appearence.theme = val ? 'dark' : 'light')
					"
				/>
			</div>
			...
			>
				...
				<Switch
					...
					@update:modelValue="
						(val) =>
							(settings.appearence.contrast = val
								? 'high'
								: 'normal')
					"
				/>
	...
	>
		<section class="flex flex-col gap-4">
			...
			<p class="text-subtext text-sm">
				Ajoutez vos propres liens qui s'afficheront dans la barre de
				navigation.
			</p>
			...
			<div class="flex flex-col gap-3 mt-4">
				...
				<div
					v-if="!settings.customization.links.length"
					class="text-center text-subtext py-4"
				>
		...
		<section class="flex flex-col gap-4">
			...
			<p class="text-subtext text-sm">
				Réorganisez l'affichage des groupes d'outils sur votre accueil.
			</p>
			...
			<div class="flex flex-col gap-3">
				...
				>
					<div class="flex items-center gap-4">
						<span
							class="text-subtext font-bold text-lg bg-surface-border w-8 h-8 rounded-full flex items-center justify-center"
							>{{ index + 1 }}</span
						>
					...
					<div class="flex flex-col gap-2">
						...
						<button
							...
							:disabled="
								index ===
								settings.customization.sectionOrders.length - 1
							"
							class="text-subtext hover:text-primary hover:bg-primary/10 rounded-lg disabled:opacity-30 disabled:cursor-not-allowed p-1 transition-all"
						>
	...
	<main v-else class="container flex flex-col gap-10 max-w-2xl mx-auto">
		<section class="flex flex-col gap-4">
			<div
				class="flex justify-between items-center border-b border-surface-border pb-2"
			>
			...
			>
				...
				<div class="flex flex-col sm:flex-row gap-4">
					<div class="flex-1 flex flex-col gap-2">
						<span
							class="text-xs font-bold uppercase tracking-wider text-subtext ml-1"
							>Année / Parcours</span
						>
					...
					<div
						...
						v-if="vencatYear && baseGroups[vencatYear]"
					>
						<span
							class="text-xs font-bold uppercase tracking-wider text-subtext ml-1"
							>Groupe</span
						>
						<Select
							:options="baseGroups[vencatYear] || [{ label: 'Aucune année sélectionnée', value: null, style: 'neutral' }]"
		...
		<section class="flex flex-col gap-4">
			...
			>
				...
				<div class="flex flex-col sm:flex-row gap-4">
					...
					<div
						class="flex-1 flex flex-col gap-2"
						v-if="planupYear && baseGroups[planupYear]"
					>
						<span
							class="text-xs font-bold uppercase tracking-wider text-subtext ml-1"
							>Groupe</span
						>
						<Select
							:options="baseGroups[planupYear] || [{ label: 'Aucun groupe sélectionné', value: null, style: 'neutral' }]"
			...
			>
				<div class="flex flex-col gap-3">
					<span class="font-medium"
						>Défilement automatique (Carrousel)</span
					>
					<div class="flex items-center gap-4">
				...
				<div class="flex flex-col gap-3">
					<span class="font-medium"
						>Canaux de communication suivis</span
					>
