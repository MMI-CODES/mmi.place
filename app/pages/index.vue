...
definePageMeta({
  ...
  description:
    "Bienvenue sur MMI Place, votre tableau de bord personnalisé pour les étudiants en MMI.",
});
...
const labels = { all: "Tous", students: "Étudiants", official: "Officiels", resource: "Ressources" };
...
<template>
  ...
  <div class="container flex justify-center gap-2">
    <Button
      ...
      :key="tabOption"
      :label="labels[tabOption as 'all' | 'students' | 'official' | 'resource']"
      :btnStyle="tab === tabOption ? 'TAB:ACTIVE' : 'TAB'"
      :handler="() => { tab = tabOption as 'all' | 'students' | 'official' | 'resource'; }"
    />
  </div>
  <main v-if="tab === 'all'" class="container space-y-12">
    <ToolRow section="pinned" />
    <ToolRow v-for="section in settings.customization.sectionOrders" :key="section" :section="section" />
  </main>
