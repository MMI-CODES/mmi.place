...
import { WrenchScrewdriverIcon, AcademicCapIcon, BookOpenIcon, BookmarkIcon } from "@heroicons/vue/24/solid";
import SectionTitle from "~/components/layout/SectionTitle.vue";

const props = defineProps<{ section: "official" | "students" | "resource" | "pinned" }>();
...
const filteredTools = computed(() => {
  switch (props.section) {
    ...
    case "pinned": {
      ...
      const all = [...tools.value.official, ...tools.value.students, ...tools.value.resource];
...
<template>
  <section v-if="filteredTools.length > 0 || (props.section !== 'pinned' && !loading)" class="container space-y-4">
    <SectionTitle :icon="icons[props.section]">{{ titles[props.section] }}</SectionTitle>
    <div v-if="loading && !tools" class="text-center text-subtext">Chargement des outils...</div>
    <div v-else-if="error" class="text-center text-on-surface">Une erreur est survenue lors du chargement des outils.</div>
    ...
    <div v-else-if="props.section !== 'pinned'" class="text-center text-subtext">Aucun outil à afficher.</div>
  </section>
</template>
