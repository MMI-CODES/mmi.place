...
watch(
  () => [settings.value.widgets.carrousel, settings.value.widgets.carrouselRate, widgets.value.length],
  () => {
...
<template>
  <header class="container grid xl:grid-cols-12">
    <div class="flex flex-col gap-2 justify-center col-span-1 p-4 md:p-8 md:space-y-4 xl:col-span-6 xl:row-1">
      <h1 v-if="session" class="text-4xl font-bold">
        Bonjour,
        <span class="text-primary underline decoration-4">{{ session?.firstName || "MMI" }}</span>
        !
      </h1>
      ...
      <p class="text-xl text-subtext max-md:hidden">
        ...
        ressources, messages campus et widgets utiles dans une interface plus complete.
      </p>
      <div class="flex gap-2 overflow-x-auto">
        <Button :icon="AdjustmentsHorizontalIcon" handler="/settings" />
        <div class="flex gap-6 ml-4 overflow-x-auto">
          <Button v-for="link in settings.customization.links" :label="link.name" btnStyle="LINK" :handler="link.url" />
        </div>
      </div>
    </div>
    ...
    >
      <div class="group shrink-0 flex flex-col items-center gap-2 w-fit" v-if="widgets.length > 1">
        <motion.div
          ...
          :animate="{ opacity: 1, scale: 1, height: index - 1 === widget ? 24 : 16 }"
